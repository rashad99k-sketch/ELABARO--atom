...
def forward_liquidity_map(df: Optional[pd.DataFrame], side: str = "BUY",
                          price: Optional[float] = None, atr: Optional[float] = None,
                          lookback: int = 250, cluster_pct: float = 0.002,
                          max_targets: int = 6) -> dict:
    """BARON Liquidity-Ahead roadmap: forward liquidity in the trade direction.

    For BUY the targets are resistances above the price; for SELL they are the
    supports below it. Distances are in ATR units so a trade manager can decide
    whether to ride (major liquidity still ahead) or protect (price at a wall).

    Pure and side-effect free: it never opens, sizes, or mutates. Returns an
    empty roadmap when data is insufficient; callers must treat a missing
    nearest target as "no invented levels" (never extrapolate a target).
    """
    empty = {
        "targets": [], "nearest": None, "nearest_distance_atr": None,
        "major_liquidity_ahead": False, "major_target": None, "roadmap_valid": False,
    }
    if df is None or len(df) < 30:
        return empty
    atr = float(atr) if atr is not None and float(atr) > 0 else 0.0
    if atr <= 0:
        try:
    ...
    price = float(price) if price and price > 0 else float(df["close"].iloc[-1])

    swing_highs, swing_lows = _swing_levels(df, lookback=lookback)
    raw_levels = swing_highs if side == "BUY" else swing_lows
    if not raw_levels:
        return empty
    eq_high, eq_low = _equal_extremes(df)
    equal = bool(eq_high if side == "BUY" else eq_low)

    clusters: List[Tuple[float, int]] = []
    for p in sorted(raw_levels):
        if clusters and abs(p - clusters[-1][0]) / p < cluster_pct:
            c = clusters[-1]
            clusters[-1] = (c[0] + (p - c[0]) / (c[1] + 1), c[1] + 1)
        else:
            clusters.append((float(p), 1))

    targets: List[Dict[str, object]] = []
    for center, touches in clusters:
        if side == "BUY" and center <= price:
            continue
        if side == "SELL" and center >= price:
            continue
        dist_atr = (center - price) / atr if side == "BUY" else (price - center) / atr
        if dist_atr <= 0:
            continue
        targets.append({
            "price": round(center, 8),
            "distance_atr": round(dist_atr, 3),
            "strength": float(touches if touches >= 2 else 1) + (1.0 if equal else 0.0),
            "touches": touches,
            "equal": equal,
        })
    targets.sort(key=lambda t: float(t["distance_atr"]))
    targets = targets[:max_targets]

    result = dict(empty)
    result["roadmap_valid"] = bool(targets)
    if targets:
        result["targets"] = targets
        result["nearest"] = targets[0]
        result["nearest_distance_atr"] = float(targets[0]["distance_atr"])
        major = [t for t in targets if float(t["distance_atr"]) >= 1.5 and (int(t["touches"]) >= 2 or bool(t["equal"]))]
        if major:
            result["major_liquidity_ahead"] = True
            result["major_target"] = major[0]
    return result
