...
The allocator is **MAX_OPEN_POSITIONS=6** by default. The production portfolio contract is **5 Technical/Institutional + 1 News = 6 maximum**. Six is a hard capacity, not a requirement: the bot opens only qualified opportunities and never creates trades just to fill slots. Live entries require exchange-native protection when `REQUIRE_NATIVE_PROTECTION_LIVE=1`. When multiple asset classes have valid candidates, selection is diversification-first and then score-ranked.
