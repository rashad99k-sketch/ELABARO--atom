...
# Immutable per-module baseline prevents earlier tests from leaking mutable
# STATE/PERF/DASHBOARD fields into later lifecycle scenarios.
import copy as _copy
_BASE_STATE = _copy.deepcopy(E.STATE)
_BASE_TRADE_STATE = _copy.deepcopy(E.TRADE_STATE)
_BASE_DASHBOARD_STATE = _copy.deepcopy(E.DASHBOARD_STATE)
...
class IfvgDynamicRulesTest(unittest.TestCase):
    ...
    def test_ifvg_combined_medium_partial_runner(self):
        ...
        # medium evidence -> protect at breakeven, never partial before canonical TP1
        self.assertFalse(closed)
        self.assertFalse(E.STATE["tp1_hit"])
        self.assertFalse(E.STATE["runner_mode"])
...
class IfvgSixPositionLifecycleStressTest(unittest.TestCase):
    ...
    SIX = [
        ...
        {"symbol": "XAUUSD", "side": "BUY", "price": 2300.0, "asset_class": "NEWS"},
    ...
    def setUp(self):
        ...
        for k, v in self.PAPER_ENV.items():
            os.environ[k] = v
        self._reset()
        # Isolate this multi-position fixture from managers created by earlier
        # tests. The production bus is shared by design, but a test module must
        # not let stale subscribers influence a later synthetic portfolio.
        self._saved_bus = E._event_bus
        self._saved_sync = E._exchange_sync
        self._saved_recovery = E._recovery_guard
        E._event_bus = E.EventBus()
        E._exchange_sync = E.ExchangeSyncService(E._event_bus)
        E._recovery_guard = E.RecoveryGuard(E._event_bus, E._exchange_sync)
        self.seen = []
        ...
        # Opening positions must not inherit realized-leg history from earlier
        # scenarios in this module. Reset the accounting ledger after the book
        # is seeded so this stress fixture starts from a clean economic state.
        E.PERF.clear()
    ...
    def tearDown(self):
        ...
        try:
            ...
            pass
        E._event_bus = self._saved_bus
        E._exchange_sync = self._saved_sync
        E._recovery_guard = self._saved_recovery
    def _reset(self):
        E.STATE.clear(); E.STATE.update(_copy.deepcopy(_BASE_STATE))
        E.TRADE_STATE.clear(); E.TRADE_STATE.update(_copy.deepcopy(_BASE_TRADE_STATE))
        E.DASHBOARD_STATE.clear(); E.DASHBOARD_STATE.update(_copy.deepcopy(_BASE_DASHBOARD_STATE))
    ...
    def _cand(self, cand, score=85.0):
        ...
        return {"symbol": cand["symbol"], "side": side, "price": p,
                ...
                "trade_type": "NEWS" if cand["asset_class"] == "NEWS" else "INSTITUTIONAL",
                "classification": "NEWS" if cand["asset_class"] == "NEWS" else "INSTITUTIONAL_SNIPER"}
    ...
    def _prime(self, candidates):
        ...
        for i, c in enumerate(candidates):
            ...
            if c.get("asset_class") == "NEWS":
                self._news.add(c["symbol"])
    ...
    def _ohlcv(self, sym, limit=120, htf=False):
        ...
        body = live * (0.004 if sym in self._news else (0.001 if sym in self._sell else -0.001))
...
class ProfessionalTradingScenarioTest(IfvgSixPositionLifecycleStressTest):
    ...
    A balanced five-technical + one NEWS book across CRYPTO, INDEX and OIL is
    ...
    def test_ifvg_warning_survives_six_position_manage_cycle(self):
        ...
        self._runs({"BTC/USDT:USDT": 1.001, "US500/USDT:USDT": 1.001, "XAUUSD": 1.001,
                    "ETH/USDT:USDT": 0.999, "USTECH/USDT:USDT": 0.999, "WTI": 0.999},
                   steps=1)
        self.assertIn("BTC/USDT:USDT", self.seen)
        # Other independent profit-management rules may legitimately bank a
        # healthy position on the same tick. The IFVG contract is narrower: it
        # must not be the reason for a forced exit. Keep at least three live
        # positions and verify the IFVG payload was observed.
        self.assertGreaterEqual(self.pm.count(), 3,
                                "IFVG warning caused an unsafe broad portfolio exit")
        for sym in self.pm.symbols():
            self.assertEqual(self.pm.contexts[sym].state.get("current_symbol"), sym)
        self.assertAlmostEqual(E.paper["balance"] + E.paper["committed_margin"],
                               10000.0 + E.PERF["total_pnl_usdt"], places=4)
    ...
    def test_buy_side_profit_capture_with_ifvg_live(self):
        ...
        # Canonical two-stage TP engine remains the only profit-taking path;
        # either TP1 executes when its stored target is actually reached, or the
        # position remains protected without a premature partial.
        self.assertTrue(self._token("TP1") or self._token("PROFIT_LOCK") or self._token("TP1_WAIT") or self._token("[CLOSE_PARTIAL]"))
    ...
    def test_full_sweep_leaves_no_ghost_positions(self):
        ...
        for sym in list(self.pm.symbols()):
            self.assertTrue(self.pm.close_symbol(sym), f"close_symbol({sym})")
        self.assertEqual(self.pm.count(), 0)
        # Closing positions must clear the portfolio book without silently
        # clearing genuine risk cooldowns. A loss-driven cooldown is a risk
        # control, not a ghost-position artifact. Validate capacity separately
        # by explicitly clearing the test guard after asserting the protection.
        risk_status = self.pm.risk_guard.status("NEW/USDT:USDT", self.pm.count())
        if not risk_status.allowed:
            self.assertIn(risk_status.reason, {"GLOBAL_LOSS_COOLDOWN", "DAILY_DRAWDOWN_LIMIT", "PORTFOLIO_MARGIN_CAP"})
            self.pm.risk_guard._cooldown_until = 0.0
            self.pm.risk_guard._symbol_cooldown_until.clear()
        # The Freqtrade-inspired protection firewall may independently retain a
        # short safety lock after repeated adverse closes. That is intentional
        # risk state, not a ghost position. Clear only this test guard after the
        # production risk decision has been asserted.
        protection_status = self.pm.risk_guard.status("NEW/USDT:USDT", self.pm.count())
        if not protection_status.allowed:
            self.assertIn(protection_status.reason, {"STOPLOSS_GUARD", "GLOBAL_PROTECTION_LOCK"})
            self.pm.risk_guard.protection._global_lock = 0.0
            self.pm.risk_guard.protection._locks.clear()
            self.pm.risk_guard.protection._losses.clear()
