...
def _news_watch(symbol, bias="BULLISH", risk=20.0, strong=True, reaction=None):
    ...
    if reaction is not None:
        E.DASHBOARD_STATE["news_reaction"] = {
            "updated_at": 1.0,
            "items": [{"symbol": symbol, "headline": "impact headline", "reaction": reaction}],
        }
...
class NewsSlotGateTest(unittest.TestCase, NewsHarnessMixin):
    ...
    def test_news_slot_waits_for_market_reaction(self):
        _news_watch("NCSKNVDA2USD/USDT:USDT", bias="BULLISH", risk=20.0)
        old_enabled = os.environ.get("NEWS_SLOT_ENABLED", "false")
        ...
        self.assertFalse(result)
        self.assertEqual(R.PORTFOLIO.count(), 0)
        self.assertEqual(E.MEMORY.get("pipeline", {}).get("execution", {}).get("last_outcome"), "news_waiting_reaction")
    ...
    def test_news_slot_on_full_path_ends_with_news(self):
        ...
        _news_watch("NCSKNVDA2USD/USDT:USDT", bias="BULLISH", risk=20.0, reaction={"move_pct": 1.2, "direction": "UP", "causality": "CONFIRMED"})
        old_enabled = os.environ.get("NEWS_SLOT_ENABLED", "false")
        os.environ["NEWS_SLOT_ENABLED"] = "true"
        R.PORTFOLIO = PortfolioManager(6, E)
        R.PORTFOLIO.bind(E)
