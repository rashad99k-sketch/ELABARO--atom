...
class ForensicFixesTest(unittest.TestCase):
    ...
    # ---- 10. Institutional fast-gate reaches canonical deep path ----
    def test_institutional_fast_gate_does_not_bypass_confirmation(self):
        E = self.engine
        q = E.ExecutionQueue()
        ...
        # The fast gate is an accelerator/prefetch hint only. It must not mint READY
        # without the canonical confirmation/zone/score/ATOM/ADX/evidence path.
        calls = []
        q._check_entry_conditions = lambda d, s, a, sy="": calls.append(sy) or True
        q.re_evaluate_all(lambda sym: _bearing_df())
        self.assertEqual(calls, ["FAST/USDT:USDT"])
        self.assertNotEqual(cand.state, E.ExecutionState.READY)
