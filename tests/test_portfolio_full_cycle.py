...
class SixSlotFullCycleTest(unittest.TestCase):
    ...
    def test_manage_all_runs_real_management_and_books_exits(self):
        ...
        # Give the management loop a deterministic, economically valid exit
        # event. A neutral/zero-ROE "PROTECT_PROFIT" signal must NOT close a
        # trade merely because the classifier says DISTRIBUTION; that would be
        # a false profit lock. Force one real synthetic-SL breach instead.
        victim = "BTC/USDT:USDT"
        E.get_ticker_safe = lambda sym, retries=0, **k: (
            E.STATE.get("entry", 60000.0) * 0.97 if sym == victim else {
                "ETH/USDT:USDT": 3000.0, "US500/USDT:USDT": 5000.0,
                "USTECH/USDT:USDT": 17000.0, "XAUUSD": 2300.0,
                "NCSKNVDA2USD/USDT:USDT": 130.0,
            }.get(sym, 100.0)
        )
        self.pm.manage_all()
        # The REAL loop executed (sync + live management + verified exit).
        before = {
...
class ProfitTakingRealPathTest(unittest.TestCase):
    ...
    def test_tp2_books_second_partial_then_finalize_releases_and_reaps(self):
        ...
        try:
            ...
            margin_after_tp2 = E.paper["committed_margin"]
            balance_after_tp2 = E.paper["balance"]
            # TP2 finalizes the entire trade exactly once.
            self.assertFalse(E.STATE.get("open"))
            self.assertEqual(margin_after_tp2, 0.0)
            self.assertGreater(balance_after_tp2, 10000.0)
