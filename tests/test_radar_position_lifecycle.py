...
class RadarPositionLifecycleTest(unittest.TestCase):
    ...
    def test_profit_taking_books_tp1_tp2_and_releases_margin(self):
        ...
        try:
            ...
            # TP1 is the canonical stored target and closes exactly 50%.
            tp1_price = float(E.STATE["tp1_price"])
            self.assertEqual(E.apply_profit_engine("BTC/USDT:USDT", tp1_price,
                                                   df, len(df) - 1, E.STATE), "TP1")
            self.assertTrue(E.STATE["tp1_hit"])
            self.assertLess(E.STATE["remaining_qty"], qty0)
            self.assertAlmostEqual(E.STATE["remaining_qty"], qty0 * 0.5, places=6)
            ...
            # TP2 closes the entire remaining 50%.
            tp2_price = float(E.STATE["tp2_price"])
            self.assertEqual(E.apply_profit_engine("BTC/USDT:USDT", tp2_price,
                                                   df, len(df) - 1, E.STATE), "TP2")
            self.assertTrue(E.STATE["tp2_hit"])
            self.assertAlmostEqual(E.STATE["remaining_qty"], 0.0, places=6)
            # TP2 already performed the single finalization transaction.
            self.assertFalse(E.STATE.get("open"))
            self.assertEqual(E.paper["committed_margin"], 0.0)
    ...
    def test_manage_all_manages_six_radar_positions_and_rotates_slots(self):
        ...
        # A management classifier alone must never manufacture a "profit" exit
        # at zero ROE. Make one radar position actually violate its synthetic
        # stop so this test verifies the real, verified exit path.
        victim = next(sym for sym in QUALIFIED if sym in self.pm.contexts)
        victim_entry = float(self.pm.contexts[victim].state.get("entry", PRICES[victim]))
        E.get_ticker_safe = lambda sym, retries=3, **k: (
            victim_entry * 0.97 if sym == victim else PRICES.get(sym, 100.0)
        )
        self.pm.manage_all()
        # The REAL management loop ran (sync + live management + verified exit).
        self.assertGreaterEqual(E.PERF["trades"], 1)
        # The deterministic victim is an intentional stop-out, so a LOSS is
        # valid here; this test verifies that a real exit is booked, not that
        # synthetic data magically produces a winner.
        self.assertGreaterEqual(E.PERF["wins"], 0)
        ...
        self.assertAlmostEqual(E.paper["balance"] + E.paper["committed_margin"],
                               10000.0 + E.PERF["total_pnl_usdt"], places=6)
