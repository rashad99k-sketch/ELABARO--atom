...
def _confirmed_reaction(symbol: str, side: str, reaction_store=None) -> Optional[dict]:
    """Return a measured post-headline reaction only when direction agrees.

    News is never treated as an order by itself. The runtime publishes short-window
    reaction telemetry; this gate requires CONFIRMED causality for the NEWS slot.
    """
    store = reaction_store or {}
    items = store.get("items", []) if isinstance(store, dict) else []
    for item in items or []:
        if not isinstance(item, dict) or str(item.get("symbol")) != str(symbol):
            continue
        reaction = item.get("reaction") if isinstance(item.get("reaction"), dict) else {}
        if str(reaction.get("causality", "")).upper() != "CONFIRMED":
            continue
        move = reaction.get("move_pct")
        try:
            min_move = float(os.getenv("NEWS_MIN_REACTION_PCT", "0.05"))
            if move is None or abs(float(move)) < min_move:
                continue
        except Exception:
            pass
        expected = "UP" if str(side).upper() == "BUY" else "DOWN"
        if str(reaction.get("direction", "")).upper() != expected:
            continue
