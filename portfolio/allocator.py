        ...
        with self._lock:
            ...
            for cand in list_cands:
                ...
                if class_bias.get(cls, 0) >= class_cap:
                    allowed = False
                    reason = f"{cls}_CAP"
                if cls != "NEWS" and allowed:
                    technical_cap = max(1, min(limit, int(os.getenv("MAX_TECHNICAL_POSITIONS", str(DEFAULT_MAX_TECHNICAL_POSITIONS)))))
                    current_technical = sum(v for k, v in class_bias.items() if k != "NEWS")
                    if current_technical >= technical_cap:
                        allowed = False
